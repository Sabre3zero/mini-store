...
export const AppForm = observer(
  ({ onSubmit, isEdit = false, initialData = null }: AppFormProps) => {
      ...
      try {
        ...
        if (coverFile) {
          ...
          try {
            ...
            throw new Error(
              `Не удалось загрузить изображение: ${uploadErr instanceof Error ? uploadErr.message : ""}`,
            );
          } finally {
        ...
        if (message.includes("categoryId обязателен")) {
          ...
          setError(
            "Не удалось определить размеры изображения. Попробуйте другое изображение.",
          );
        } else {
          setError(`${message}`);
        }
    ...
    return (
      <div className={styles.formContainer}>
        <header className={styles.header}>
          <h2>
            {isEdit ? "Редактирование приложения" : "Создание приложения"}
          </h2>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.formGroup}>
            ...
            <select
              ...
              onChange={handleChange}
              disabled={
                isLoading || uploadingImage || !userStore.categories.length
              }
            >
